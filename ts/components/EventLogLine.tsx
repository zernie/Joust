...
export interface EventLogLineProps
	extends CardDataProps,
		...
		EventLogItemData,
		StripBattletagsProps,
		React.ClassAttributes<EventLogLine> {
	inactive: boolean;
	first?: boolean;
}
...
export default class EventLogLine extends React.Component<
	EventLogLineProps,
	EventLogLineState
> {
	...
	public render(): JSX.Element {
		...
		const entity = (
			<EventLogCard
				...
				cardId={
					...
					this.props.cardOracle.get(this.props.entityId)
				}
			/>
		);
		const target = (
			<EventLogCard
				...
				cardId={
					this.props.cardOracle &&
					this.props.cardOracle.get(this.props.targetId)
				}
			/>
		);
		...
		return (
			<div className={classNames.join(" ")}>
				{characters ? <pre>{characters}</pre> : null}
				{parts}
			</div>
		);
	}
	...
	private parseLine(strings: any): string {
		switch (this.props.type) {
			...
			case LineType.Draw:
				return this.state.targetData
					? "%player% draws %entity% from %source%"
					: "%player% draws %entity%";
			case LineType.Summon:
				return this.state.entityData &&
					this.state.entityData.type === "WEAPON"
					? "%source% creates %entity%"
					: "%source% summons %entity%";
			case LineType.Replace:
				return "%entity% replaces %target%";
			case LineType.ArmorBuff:
				return this.state.targetData
					? "%entity% gains %data% armor from %source%"
					: "%entity% gains %data% armor";
			case LineType.AttackBuff:
				return this.state.targetData
					? "%entity% gains +%data% attack from %source%"
					: "%entity% gains +%data% attack";
			case LineType.HealthBuff:
				return this.state.targetData
					? "%entity% gains +%data% health"
					: "%entity% gains +%data% health from %source%";
			...
			case LineType.Attack:
				return this.props.data
					? "%entity% attacks %target% for %data%"
			...
			case LineType.Get:
				return this.state.targetData
					? "%player% receives %entity% from %source%"
					: "%player% receives %entity%";
			case LineType.GetToDeck:
				return this.state.targetData
					? "%entity% from %source% is added to %player%'s deck"
					: "%entity% is added to %player%'s deck";
			case LineType.Trigger:
				return "%entity% triggers";
			case LineType.Damage:
				return this.state.targetData &&
					this.state.targetData.type == "WEAPON"
					? this.state.entityData
						? "%target% loses %data% durability from %entity%"
						: "%target% loses %data% durability"
					: this.props.data
						? "%entity% damages %target% for %data%"
						: "%entity% hits %target%";
			case LineType.Healing:
				return "%entity% heals %target% for %data%";
			case LineType.Remove:
				return this.state.targetData
					? "%source% removes %entity%"
					: "%entity% is removed";
			case LineType.Mulligan:
				return "%player% mulligans %entity%";
			case LineType.Silenced:
				return this.state.targetData
					? "%entity% is silenced by %source%"
					: "%entity% is silenced";
			case LineType.Frozen:
				return this.props.data
					? this.state.targetData
						? "%source% freezes %entity%"
						: "%entity% gets frozen"
					: "%entity% is no longer frozen";
			case LineType.Steal:
				return this.state.targetData
					? "%player% steals %entity% using %source%"
					: "%player% steals %entity%";
			...
			case LineType.CantBeDamaged:
				...
				return this.props.data
					? this.state.targetData
						? "%entity% gains %status% from %source%"
						: "%entity% gains %status%"
					: "%entity% loses %status%";
			case LineType.Cthun:
			...
			case LineType.Play:
				...
				return this.state.targetData
					? this.state.entityData &&
					  this.state.entityData.type === HERO_POWER
						? "%player% uses %entity% targeting %target%"
						: "%player% plays %entity% targeting %target%"
					: this.state.entityData &&
					  this.state.entityData.type === HERO_POWER
						? "%player% uses %entity%"
						: "%player% plays %entity%";
			case LineType.StatsBuff:
				...
				return this.state.targetData
					? "%entity% gains %stats% from %target%"
					: "%entity% gains %stats%";
		}
	}
