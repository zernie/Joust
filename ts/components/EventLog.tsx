...
export interface EventLogProps
	extends CardDataProps,
		CardOracleProps,
		StripBattletagsProps,
		React.ClassAttributes<EventLog> {
	state: GameState;
	tail: HistoryEntry;
	currentTime: number;
	isHidden?: boolean;
...
> {
	...
	public shouldComponentUpdate(
		nextProps: EventLogProps,
		nextState: EventLogState,
	) {
		return (
			(this.props.tail &&
				this.props.tail.state !== nextProps.tail.state) ||
	...
	private parseHistory(tail: HistoryEntry) {
		...
		if (current) {
			lines.push({
				type: LineType.Turn,
				data: -1,
				time: current.state.time,
			});
			while ((next = current.next)) {
				this.analyzeGameStateDiff(current.state, next.state).forEach(
					(d) => lines.push(d),
				);
				current = next;
			}
		...
		for (let i = 0; i < input.length - 1; i++) {
			...
			if (curr.targetId == next.targetId) {
				if (
					...
					((curr.type == LineType.AttackBuff &&
						next.type == LineType.HealthBuff) ||
						(curr.type == LineType.HealthBuff &&
							next.type == LineType.AttackBuff))
				) {
					output.push({
						type: LineType.StatsBuff,
						time: curr.time,
	...
	private getTurn(prev: GameState, curr: GameState): EventLogItemData {
		...
		if (
			...
			(!prev
				.getPlayers()
				.every(
					(x) => x.getTag(GameTag.MULLIGAN_STATE) == Mulligan.DONE,
				) &&
				curr
					.getPlayers()
					.every(
						(x) =>
							x.getTag(GameTag.MULLIGAN_STATE) == Mulligan.DONE,
					))
		) {
			...
			this.setLidPlayer(
				lid,
				curr,
				(p) => p.getTag(GameTag.CURRENT_PLAYER) == 1,
			);
		}
		if (lid.type) {
			return lid;
		...
		curr.descriptors
			...
			.forEach((d) => {
				...
				if (type == BlockType.ATTACK) {
					...
					(type == BlockType.POWER || type == BlockType.TRIGGER) &&
					lid.entityId
				) {
					if (
						d.type == BlockType.TRIGGER &&
						entity &&
					...
					this.setLidPlayer(
						lid,
						curr,
						(p) => p.playerId == entity.getController(),
					);
					push(LineType.Play);
				}
			});
	...
	private analyzeDiffs(state: GameState): EventLogItemData[] {
		...
		state.diffs.forEach((diff: GameStateDiff) => {
			...
			if (diff.tag == GameTag.ZONE) {
				if (
					diff.previous == Zone.DECK &&
					diff.current == Zone.HAND &&
					lid.targetId &&
	...
	private getInPlayLine(
		diff: GameStateDiff,
		state: GameState,
		entity: Entity,
		lid: EventLogItemData,
	): LineType {
		switch (diff.tag) {
			case GameTag.ATK:
			...
			case GameTag.WEAPON:
				if (diff.current) {
					...
					this.setLidEntity(
						lid,
						state,
						entity && entity.getTag(GameTag.HERO_ENTITY),
					);
					...
					case Zone.GRAVEYARD:
						return (
							[
								...
								CardType.HERO,
								CardType.WEAPON,
							].indexOf(cardType) !== -1 && LineType.Death
						);
					case Zone.SETASIDE:
						return (
							[
								...
								CardType.HERO,
								CardType.HERO_POWER,
							].indexOf(cardType) !== -1 && LineType.Remove
						);
				}
				break;
			case Zone.DECK:
					...
					case Zone.PLAY:
						return (
							[
								...
								CardType.HERO,
								CardType.HERO_POWER,
							].indexOf(cardType) !== -1 && LineType.Summon
						);
				}
				break;
			case null:
